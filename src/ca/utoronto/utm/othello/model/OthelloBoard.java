...
public class OthelloBoard implements Visitable{
	...
	/**
	 * Return any move the player can do
	 * 
	 * @param player which player we want to look for a move
	 * @return a move that the player has
	 */
	public Move findaMove(char player) {
		for (int row = 0; row < this.dim; row++) {
			for (int col = 0; col < this.dim; col++) {
	...
	/**
	 * 
	 * return the player can move at the position
	 */
	public boolean canMove(int row, int col, char player) {
		if (player == EMPTY) {
			return false;
		}
		for (int drow = -1; drow <= 1; drow++) {
	...
	public void accept(OthelloAndOthelloBoardVisitor visitor){
		visitor.visit(this);
	}
	
	/**
	 * 
	 * return if the given position is at a corner
	 */
	public boolean isCorner(int row, int col) {
		if (row == 0 || row == this.dim-1) {
			if (col == 0 || col == this.dim-1) {
				return true;
			}
		}
		return false;
	...
	/**
	 * 
	 * return if the given position is at a side
	 */
	...
	/**
	 * 
	 * return if the given position is at the middle 4*4 grid
	 */
	public boolean is4times4(int row, int col){
		if (row == 2 || row == this.dim-3) {
			if (2 <= col && col <= this.dim-3) {
				return true;
			}
		}
		if (col == 2 || col == this.dim-3) {
			if (2 <= row && row <= this.dim-3) {
				return true;
			}
		}
		return false;
